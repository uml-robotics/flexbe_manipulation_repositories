# FlexBE States and Behaviors for end_effector
...
This raw repository has several folders and files with the generic name `end_effector`.
...
and change the relevant `end_effector` text to `my_new_project_name` as needed.
...
## Example States in `end_effector_flexbe_states`
...
## Example Behaviors in `end_effector_flexbe_behaviors`
...
[`copy_behavior`](end_effector_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run end_effector_flexbe_behavior copy_behavior` to see the usage guide. 
