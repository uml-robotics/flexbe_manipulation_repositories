# FlexBE States and Behaviors for gpd
...
This raw repository has several folders and files with the generic name `gpd`.
...
and change the relevant `gpd` text to `my_new_project_name` as needed.
...
## Example States in `gpd_flexbe_states`
...
## Example Behaviors in `gpd_flexbe_behaviors`
...
[`copy_behavior`](gpd_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run gpd_flexbe_behavior copy_behavior` to see the usage guide. 
