# FlexBE States and Behaviors for move_group
...
This raw repository has several folders and files with the generic name `move_group`.
...
and change the relevant `move_group` text to `my_new_project_name` as needed.
...
## Example States in `move_group_flexbe_states`
...
## Example Behaviors in `move_group_flexbe_behaviors`
...
[`copy_behavior`](move_group_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run move_group_flexbe_behavior copy_behavior` to see the usage guide. 
