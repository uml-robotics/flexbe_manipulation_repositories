# FlexBE States and Behaviors for mtc
...
This raw repository has several folders and files with the generic name `mtc`.
...
and change the relevant `mtc` text to `my_new_project_name` as needed.
...
## Example States in `mtc_flexbe_states`
...
## Example Behaviors in `mtc_flexbe_behaviors`
...
[`copy_behavior`](mtc_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run mtc_flexbe_behavior copy_behavior` to see the usage guide. 
