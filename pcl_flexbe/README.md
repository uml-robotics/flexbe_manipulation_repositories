# FlexBE States and Behaviors for pcl
...
This raw repository has several folders and files with the generic name `pcl`.
...
and change the relevant `pcl` text to `my_new_project_name` as needed.
...
## Example States in `pcl_flexbe_states`
...
## Example Behaviors in `pcl_flexbe_behaviors`
...
[`copy_behavior`](pcl_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run pcl_flexbe_behavior copy_behavior` to see the usage guide. 
