...
from pcl_ros2.srv import PassthroughFilter as SrvType  # request: input, response: filtered

class PassthroughServiceState(EventState):
    """
    Calls /passthrough_filter to downsample a cloud.

    -- service_timeout   float     Timeout for service discovery (sec, default: 5.0)
    -- service_name      str       Service name (default: '/passthrough_filter')
    ...
    <= failed
    """
    def __init__(self, service_timeout=5.0, service_name='/passthrough_filter', lower_limit=0.0, upper_limit=1.0, field='z'):
        super().__init__(
    ...
    def on_enter(self, userdata):
        ...
        request.upper_limit = float(self._params['upper_limit'])
        request.lower_limit = float(self._params['lower_limit'])
        request.field = str(self._params['field'])
