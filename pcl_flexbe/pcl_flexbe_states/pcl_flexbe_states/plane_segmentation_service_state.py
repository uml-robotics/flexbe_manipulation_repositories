...
from pcl_ros2.srv import PlaneSegmentation as SrvType # request: input, use_voxel, leaf_size, distance_threshold, max_iterations
...
class PlaneSegmentationServiceState(EventState):
    ...
    def __init__(self, service_timeout=5.0, service_name='/plane_segmentation', use_voxel=True, leaf_size=0.01, distance_threshold=0.01, max_iterations=1000):
        super().__init__(outcomes=['finished', 'failed'],
                            input_keys=['cloud_in'],
                            output_keys=['without_plane', 'plane_indices', 'plane_coefficients', 'inlier_count']
        )
        self._params = dict(
            use_voxel=use_voxel,
