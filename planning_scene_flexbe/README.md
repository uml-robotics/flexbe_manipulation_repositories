# FlexBE States and Behaviors for planning_scene
...
This raw repository has several folders and files with the generic name `planning_scene`.
...
and change the relevant `planning_scene` text to `my_new_project_name` as needed.
...
## Example States in `planning_scene_flexbe_states`
...
## Example Behaviors in `planning_scene_flexbe_behaviors`
...
[`copy_behavior`](planning_scene_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run planning_scene_flexbe_behavior copy_behavior` to see the usage guide. 
