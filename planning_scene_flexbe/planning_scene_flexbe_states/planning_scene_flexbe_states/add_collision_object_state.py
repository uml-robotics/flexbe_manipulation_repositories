...
class GetPointCloudServiceState(EventState):
    ...
    <= failed
    """
    def __init__(self, service_name='/add_collision_object', x=0.0, y=0.0, z=0.0, l=0.0, w=0.0, h=0.0):
    ...
    def execute(self, userdata):
        ...
        # Check for success, no userdata to write
        try:
    ...
    def on_enter(self, userdata):
        ...
        if not self._srv.is_available(self._service_name):
            Logger.logerr(f"[{type(self).__name__}] Service '{self._service_name}' not available.")
